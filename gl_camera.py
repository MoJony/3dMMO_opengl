...
class FirstPersonCamera(object):
    ...
    def __init__(self, position=(0, 0, 0), movement_speed=DEFAULT_MOVEMENT_SPEED, mouse_sensitivity=DEFAULT_MOUSE_SENSITIVITY, y_inv=True):
    ...
    def update(self, delta_time,dx,dy):
        """Update camera state"""
        #print(self.__input_handler.dx,  self.__input_handler.dy)   # mojo
        ...
        if self.__input_handler.pressed['W']:
            self.move_forward(delta_time * self.movement_speed)

        if self.__input_handler.pressed['S']:
            self.move_backward(delta_time * self.movement_speed)

        if self.__input_handler.pressed['A']:
            self.move_left(delta_time * self.movement_speed)

        if self.__input_handler.pressed['D']:
            self.move_right(delta_time * self.movement_speed)

        if self.__input_handler.pressed['UP']:
            self.move_up(delta_time * self.movement_speed)

        if self.__input_handler.pressed['DOWN']:
            self.move_down(delta_time * self.movement_speed)

    def draw(self):
        """Apply transform"""

        glRotatef(self.__pitch, 1.0, 0.0, 0.0)
        glRotatef(self.__yaw, 0.0, 1.0, 0.0)
        glTranslatef(*self.__position)